...
| 4.5b | **JTX shoot two finishes and only two.** Probed their uploads for B, GB, MB, BR, C and every other plausible code — all 404. There is no plain all-black render; the black option is Black Milled, where the machined spoke faces are bare aluminium by design. The page now says so, because the bright edges otherwise read as a broken photo. | 🟡 Ask JTX whether a plain-black render exists internally. |
...
| 4.9 | **Series coverage:** 20 models have single-series art (19 in both finishes), 14 have a complete dually front+rear pair. Ace has neither and falls back to the catalogue photo under "More styles". | 🟡 |
